...
ACTION_DICT = {
    ...
    2: 'move -0.5', # Move backwards at half speed
    3: 'turn -0.5',  # Turn to the left
    4: 'turn 0.5', # turn to the right
    5: 'turn 0', # stop turning
    6: 'jump 1',  # start jumping
    7: 'jump 0' # stop jumping
}
                   ...
                   <AgentHandlers>
                       ...
                       <ObservationFromGrid>
                        ...
                        <RewardForTouchingBlockType>
                            ...
                            <Block reward="25" type="emerald_block"/>
...
def drawPath():
    return random.choice([drawStraightPath(), drawPath2(), drawPath3(), drawPath4()])
...
def get_action(obs, q_network, epsilon, allow_break_action):
    ...
    with torch.no_grad():
        ...
        # move randomly with probability e, greedily otherwise. If past 12 moves had no breaks, move randomly
        if (random.random() > epsilon):
            # Select action with highest Q-value
            action_idx = torch.argmax(action_values).item()
        else:
            action_idx = random.choice([i for i in range(action_values.size()[1]) if action_values[0][i] > float("-inf")])

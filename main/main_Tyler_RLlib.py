...
##
    ...
    def drawPath(self, pathID):
        ...
        if pathID == 0:
            ...
            path += "<DrawBlock x='0' y='1' z='5' type='gold_block' />"
            path += "<DrawBlock x='0' y='2' z='3' type='diamond_block' />" 
            path += "<DrawBlock x='6' y='1' z='5' type='emerald_block' />"

        elif pathID == 5: #this is the world platform obstacle lenient
            a1=8
            for i in range(a1):
                for k in range(a1):
                    path += f"<DrawBlock x='{i}' y='1' z='{k}' type='cobblestone' />" \
            
         
            for k in range(3,5):
                a2=a1+1
                for j in range(-1,9):
                    path += f"<DrawBlock x='-1' y='{k}' z='{j}' type='cobblestone' />" 
                    path += f"<DrawBlock x='{a2}' y='{k}' z='{j}' type='cobblestone' />" 
                    path += f"<DrawBlock x='{j}' y='{k}' z='-1' type='cobblestone' />" 
                    path += f"<DrawBlock x='{j}' y='{k}' z='{a2}' type='cobblestone' />" 
                    
            
           

            for i in range(5):#obstacles
                x=randint(0,a1-1)
                z=randint(0,a1-1)
                if x==0 and z==0:
                    x=1
                    z=1
                path += f"<DrawBlock x='{x}' y='2' z='{z}' type='cobblestone' />" \
            
           # path += "<DrawBlock x='4' y='1' z='4' type='diamond_block' />"
            path += "<DrawBlock x='4' y='2' z='4' type='gold_block' />"
            path += "<DrawBlock x='2' y='2' z='2' type='diamond_block' />"
           # path += "<DrawBlock x='0' y='2' z='3' type='diamond_block' />" 
            path += "<DrawBlock x='6' y='2' z='6' type='emerald_block' />"

        elif pathID == 6: #this is the world platform obstacle lenient
            a1=8
            for i in range(a1): #makes bottom grid
                for k in range(a1):
                    path += f"<DrawBlock x='{i}' y='1' z='{k}' type='cobblestone' />" \
